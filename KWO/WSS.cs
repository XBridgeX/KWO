            ...
            switch (jj["action"].ToString())
            {
                case "runcmdrequest":
                    try
                    {
                        ...
                        sockets[socket] = j.id;
                        tmpid = j.id;
                        Form1.console.AppendText($"[WS] runcmd >> {j.cmd}" + Environment.NewLine);
                        runcmd(j.cmd);
                    }
                ...
                case "sendtext":
                    try
                    {
                        Form1.console.AppendText($"[WS] sendtext >> {j.text}" + Environment.NewLine);
                        SendAllText(j.text);
                    }
                ...
                case "start":
                    ...
                    break;
                default:
                    Form1.console.AppendText($"[WS][ERROR] unknow pack >>{jj["action"]}<<"+Environment.NewLine);
                    break;
            }
        }
